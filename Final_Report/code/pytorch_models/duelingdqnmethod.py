...
# Hyperparameters
BATCH_SIZE = 1024
GAMMA = 0.9 #0.9 for highway, 0.8 for merge
EPS_START = 0.999
EPS_END = 0.05
EPS_DECAY = 400
...
class DuelingDQN(nn.Module):
    def __init__(self, inputs, outputs):
        ...
        self.value_stream = nn.Sequential(
            nn.Linear(512, 256),
            nn.LeakyReLU(),
            nn.Linear(256, 1)  # Outputs a single value V(s)
        )
        # Advantage stream
        self.advantage_stream = nn.Sequential(
            nn.Linear(512, 256),
            nn.LeakyReLU(),
            nn.Linear(256, outputs)  # Outputs A(s, a) for each action
        )
    def forward(self, x):
        x = x.view(x.size(0), -1)
        ...
        # Combine the streams to get Q(s, a)
        # Q(s,a) = V(s) + (A(s,a) - mean(A(s,a)))
        # The mean is taken over the actions (dim=1)
